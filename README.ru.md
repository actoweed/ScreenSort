...
- **Полнотекстовый OCR-поиск** — извлекает текст с каждого скриншота, индексирует в SQLite, поиск через меню трея
...
- **GPU не нужен** — работает на CPU; при первом запуске скачиваются модели ~600 МБ
...
- При первом запуске модели ИИ скачаются автоматически (~600 МБ)
...
- **Поиск по тексту** — поиск по тексту на всех отсортированных скриншотах
...
2. **[EasyOCR](https://github.com/JaidedAI/EasyOCR)** считывает видимый текст со скриншота и сохраняет его в локальную SQLite FTS5 базу для поиска.
3. **[imagehash](https://github.com/JohannesBuchner/imagehash)** вычисляет перцептивный хеш каждого файла — дубли тихо пропускаются.
4. **[watchdog](https://github.com/gorakhargosh/watchdog)** слушает события файловой системы — никакого поллинга, ноль CPU в простое.
...
│   ├── ocr.py                    # EasyOCR + SQLite FTS5 индекс
